package client

import (
	"encoding/json"
	"time"

	"github.com/cnlesscode/gotool"
	"github.com/gorilla/websocket"
)

func Regist(addr, mainKey, registerAddr string, onChangeCallback func(msg map[string]int)) {
	firstConnect := true
	go func() {
		// 初始化连接地址
		url := "ws://" + addr + APIBaseURL
		if onChangeCallback == nil {
			url += "register&mainKey=" + mainKey + "&addr=" + registerAddr
		} else {
			url += "register&mainKey=" + mainKey + "&listen=true&addr=" + registerAddr
		}
	RegisterLoop:
		// 创建连接
		conn, _, err := websocket.DefaultDialer.Dial(url, nil)
		if err != nil {
			// 失败重连
			time.Sleep(time.Second)
			goto RegisterLoop
		}

		// 启动心跳协程
		stopHeartbeat := make(chan struct{})
		go func() {
			ticker := time.NewTicker(HeartbeatInterval * time.Second)
			defer ticker.Stop()
			for {
				select {
				case <-ticker.C:
					if err := conn.WriteMessage(websocket.PingMessage, nil); err != nil {
						return
					}
				case <-stopHeartbeat:
					return
				}
			}
		}()

		// 监听 Pong 消息
		conn.SetPongHandler(func(appData string) error {
			return nil
		})

		if onChangeCallback != nil && firstConnect {
			// 第一次执行时先查询服务数据并执行监听函数
			data, err := Get(addr, mainKey)
			if err != nil {
				gotool.LogFatal("Failed to listen the serverFinder. Error Code 100001")
				return
			}
			message := make(map[string]int)
			if err := json.Unmarshal([]byte(data), &message); err != nil {
				gotool.LogFatal("Failed to listen the serverFinder. Error Code 100002")
				return
			}
			onChangeCallback(message)
			firstConnect = false
		}

		// 读取消息保持连接
		for {
			_, messageByte, err := conn.ReadMessage()
			// 服务端断开连接
			if err != nil {
				close(stopHeartbeat)
				conn.Close()
				break
			}
			if onChangeCallback != nil {
				// 数据变化消息
				message := make(map[string]int)
				if err := json.Unmarshal(messageByte, &message); err != nil {
					continue
				}
				onChangeCallback(message)
			}
		}

		// 断线重连
		time.Sleep(time.Second)
		goto RegisterLoop

	}()
}
